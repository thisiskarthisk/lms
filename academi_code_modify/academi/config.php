...
if (isset($_POST['theme_switch'])) {
    $theme = $_POST['theme_switch'] === 'on' ? 'local/purpleproflujotheme' : 'local/proflujotheme';
    setcookie('moodle_theme', $theme, time() + (86400 * 30), "/"); // Store for 30 days
    $CFG->theme = $theme;
    header("Location: " . $_SERVER['PHP_SELF']); // Refresh page to apply theme
    exit();
} elseif (isset($_COOKIE['moodle_theme'])) {
    $CFG->theme = $_COOKIE['moodle_theme'];
}
